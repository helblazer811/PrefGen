...
from gan_control.evaluation.fecnet_expression import calc_fecnet_expression_from_tensor_images
...
def make_attributes_df(model, trainer, attributes_df_save_path, batch_size=40, number_of_samples=10000, align_3d=True):
    attributes_df = pd.DataFrame(columns=['latents', 'latents_w', 'emb', 'age', 'orientation', 'expression_q', 'fecnet_expression', 'hair', 'gamma3d', 'expression3d', 'orientation3d'])
    lm3D = load_lm3d()
    ...
    for batch_num in tqdm(range(number_of_samples // batch_size)):
        ...
        fecnet_expression = calc_fecnet_expression_from_tensor_images(trainer.fecnet_expression_class, out)
        ...
        for latent_i, latent_w_i, age_i, yaw, pitch, roll, expression_q_i, fecnet_expression, hair_i, \
            ...
            df_entry = {
                ...
                'fecnet_expression': fecnet_expression,
                'hair': hair_i[0].numpy(),
